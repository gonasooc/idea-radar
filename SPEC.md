...
전부 **경고 수준이고 job을 빨간불로 만들지 않는다**(4.2와 같은 이유). 다만 그 한계를 알고 쓸 것 — 성공한 job에 붙은 `::warning::`은 이메일이 오지 않으므로, 이 검출기는 "Actions 로그를 볼 때 눈에 띄는" 수준이지 1차 감지기가 아니다. 1차 감지기로 올리려면 판정 결과를 manifest에 실어 사이트 상태 줄에 노출해야 한다.
