...
- `lastSeenAt` 갱신은 페이지를 열 때가 아니라 **페이지가 화면에 보이는 상태로 일정 시간(120초)이 지났을 때 자동으로** 한다. 열자마자 갱신하면 실수로 닫았을 때 그날 것을 영영 못 본다. 명시적 "여기까지 봤음" 버튼은 두지 않는다(v1에서 넣었다가 제거 — 자동 갱신으로 충분하다는 사용자 결정).
...
왜 소스 그룹이 아닌가: 하루 25~30건이면 그룹핑이 오히려 스캔을 방해한다. 소스별로 나뉘면 "오늘 전체를 다 봤나"를 알기 어렵다. 소스별 그룹 보기는 두지 않는다. 대신 **소스 필터 칩을 검색 모드만이 아니라 기본 화면에도 상시 노출**해, 검색 전에도 특정 소스만 걸러 볼 수 있게 한다(필터 활성 시 헤더에 `새 항목 N건 · 표시 M건`으로 전체 대비 표시 수를 남긴다).
