...
8. 【필수】 `row.description`의 **flight 참조를 푼다.** 값이 `/^\$[0-9a-f]+$/`(예: `"$1e"`)이면 그건 설명이 아니라 다른 flight 행을 가리키는 참조다. 길이가 어느 선을 넘는 문자열은 인라인되지 않고 별도 행으로 빠지기 때문이다. 2026-08-05 실측: `cat=web` 12건 중 **6건이 참조**였다.

   참조를 풀려면 flight 전체를 행 단위로 스캔해 표를 만든 뒤 id로 찾는다. 행 형식은 두 가지다:

     {행ID}:T{길이},{본문}      ← 길이는 **바이트**(hex). 본문 뒤에 구분자가 없다.
     {행ID}:{값}\n              ← 그 외 전부 개행 종료

   · **길이는 문자가 아니라 바이트다.** 실측 `1e:T1243,` = 0x1243 = 4675바이트 = 2647자. 문자 수로 자르면 한글에서 다음 행을 먹는다.
   · **T행 뒤에는 개행이 없다.** 선언된 바이트가 끝나는 그 자리에서 다음 행이 바로 시작한다. `\n`으로 split 하면 안 되고 앞에서부터 순차 스캔해야 한다.
   · **행 ID는 비어 있을 수 있다** (`:HL["/a.css","style"]` 같은 hint 행). `[0-9a-f]+`로 받으면 거기서 스캔이 멈춰 뒤쪽 T행을 통째로 놓친다.
   · 해석에 실패하면 **`""`로 떨어뜨리고 경고를 남긴다.** `"$1e"`를 그대로 저장하면 안 된다 — 항목은 불변이라 영영 못 고친다.

   구현은 `src/flight.ts`의 `scanRows()` / `derefText()`.

9. 각 row를 fieldMapping대로 공통 스키마로 변환한다. 변환 전 `row.id`가 UUID 정규식
     /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i
   에 맞고 `row.title`이 비어있지 않은지 확인하고, 아니면 그 row는 건너뛴다.

10. web·game 두 카테고리 결과를 합치고 `id` 기준 Set으로 중복 제거한다(교차 중복은 실측상 없지만 방어적으로 유지). `collectedAt`은 실행 시작 시 한 번 계산한 값을 모든 아이템에 동일하게 넣는다.
...
- description  = parseSteps 8에서 **참조를 푼 뒤의** 문자열. 그 다음
                   row.description.replace(/\r\n/g, '\n').trim()
                 참조를 풀지 않으면 `"$1e"` 같은 토큰이 그대로 저장된다(2026-08-05 이전 아카이브 27건 중 10건이 이 상태였다).
...
배치 전체 단언:
 ...
 H19. **어떤 아이템의 description도 /^\$[0-9a-f]+$/ 에 매치되면 안 된다 → 매치되면 소스 실패.**
      flight 참조(parseSteps 8)를 풀지 못하고 토큰을 그대로 저장하려는 상태다.
      다른 단언과 달리 경고가 아니라 실패인 이유: 항목은 불변이라 한 번 커밋되면 되돌릴 수 없다.
      해석 실패 건수 > 0 자체는 경고로 남기고 그 항목의 description만 ''로 떨어뜨린다.
...
- **【치명적】 dehydrated JSON을 파싱했다고 끝이 아니다.** 긴 문자열은 그 안에 없고 `"$1e"` 참조만 있다(parseSteps 8). `JSON.parse`가 성공하고 키도 다 있고 rows 개수도 맞으므로 **모든 healthCheck를 통과한다.** 2026-07-29 스펙 작성 시 예제로 본 `rows[0]`이 마침 짧아 인라인이었던 탓에 이 함정을 놓쳤고, 그 결과 2026-08-05까지 아카이브 27건 중 10건의 description이 `$1e`·`$1f`·`$20` 같은 토큰으로 저장됐다(7건은 목록에서 재수집해 복구, 3건은 창 밖으로 밀려나 `""` 처리). H19가 이 재발을 막는다.
- **미해결: `$$` 이스케이프.** RSC는 값이 진짜로 `$`로 시작할 때 `$$`로 이스케이프하는 것으로 알려져 있으나, 실측 페이로드에서 `$$` 등장 횟수가 0이라 확인하지 못했다. 확인 못 한 변환을 넣으면 `$$$ 수익` 같은 정상 문자열을 훼손할 수 있으므로 **지금은 손대지 않는다**(가공 금지). description이 `$$`로 시작하는 사례가 실제로 관측되면 그때 다시 판단한다.
