...
7. **교차 파일 쓰기는 월 샤드 → seen 인덱스 순서다.** 반대로 쓰면 seen 성공 뒤 샤드 실패 시 항목이 영구 누락된다. 샤드를 먼저 쓰면 seen 실패는 다음 병합이 샤드의 id를 보고 자가복구할 수 있다. `src/integrity.ts`의 아카이브↔seen 전량 일치 검증을 약화하지 않는다.
